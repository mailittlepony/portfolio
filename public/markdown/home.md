# Maïli Truong
---
## Table of Content:
- **[Six featured projects](#six-featured-projects)**
- **[Other projects](#other-projects)**
- **[Why do I do what I do?? Here's a pretty long answer!](#why-do-i-do-what-i-do-heres-a-pretty-long-answer)**
- **[Contact](#contact)**
- **[Fun facts](#fun-facts)**

If you are interested in my work, you can [View My CV](/portfolio/cv.pdf) or **reach out**: maili.truong@edu.devinci.fr !

## Six featured projects 
---
![mirrormorphose](./images/mirrormorphose/mirror.png)
![flowergame](./images/SmallGameboy/FlowerGame.png)
![ShadowBallet](./images/ShadowBallet/shadow.jpeg)
![rayedit](./images/rayedit/rayedit.png)
![gaze_tracker](./images/gaze_tracker/gaze.png)
![mathexploration](./images/MathExploration/MathExploration.png)

If you want to see more projects continue reading !

## Other projects
---
**[SatelliteAntenna](https://github.com/mailittlepony)**  
_Really fun antenna made during bootcamp to retreive data from a dead satellite._  
- **Technologies**: teamwork, fun  
- **License**: IFT License 2.0  
- **Last Updated**: last week  
- **Image**: ./images/SatelliteAntenna/SatelliteAntenna.jpeg

---
**[simpleECS](https://github.com/mailittlepony)**  
_Simple ECS C++ library._  
- **Technologies**: C++, ECS  
- **License**: Apache License 2.0  
- **Last Updated**: last week  
- **Image**: ./images/SimpleECS/SimpleECS.png

---
**[mnist](https://github.com/mailittlepony)**  
_MNIST handwritten digit classification using Tinygrad._  
- **Technologies**: Python, AI, webgpu, tinygrad  
- **License**: Apache License 2.0  
- **Last Updated**: last week  
- **Image**: ./images/mnist/mnist.png

---
**[raymarching](https://github.com/mailittlepony)**
_Interactive weather visualization using ray marching._
- **Technologies**: glsl, raymarching  
- **License**: Apache License 2.0  
- **Last Updated**: last week  
- **Image**: ./images/Droppy/droppy2.png

---
**[stackrush](https://github.com/mailittlepony)**  
_2D remake of the stack game._  
- **Technologies**: Python  
- **License**: Apache License 2.0  
- **Last Updated**: 5 days ago 
- **Image**: ./images/StackRush/StackRush2.png

---
**[transconnect](https://github.com/mailittlepony)**  
_A C# console application for managing your society._  
- **Technologies**: C#
- **License**: Apache License 2.0  
- **Last Updated**: last week  
- **Image**: ./images/Transconnect/TransConnect.png

## Why do I do what I do?? Here's a pretty long answer!
---

**Toward the Density of Things**

We live in an era of prediction, modeling, and generalization.
AI turns the world into probabilities.
Everything can be data.
Every behavior can be a pattern.
Every self can be a model.
Thus I propose an ontology: **the world as density**. Every object, every identity, every memory is dense — in matter, in information, in relations, and in potentiality.

Just like neural networks, I see identity as an evolving model: neither a fixed core nor a multiplicity. It is a dynamic system whose weights shift with each new input. The self becomes an architecture of memories, environments, and relationships — a dense model whose outputs may change while its underlying topology persists. Experiences move us through this latent space, yet the structure that shapes our possible selves is seeded early. **Thus human behavior, like AI behavior, arises from patterns, biases, and histories embedded in the model that constitutes us, making the human sometimes predictable.**

![vision](./images/Vision/vision.jpeg)

My work seeks to expose and question these layers: **the hidden densities of identity**, the invisible architectures of relation, and the computational metaphors that increasingly frame how we understand ourselves. Through AI and emerging technologies, **I explore the boundaries of selfhood, connection, and interpretation — inviting new ways of perceiving what we are, what we might become, and how we might exceed our own predictability.**

## Contact 
---
- Email: maili.truong@edu.devinci.fr
- [GitHub](https://github.com/mailittlepony)
- [LinkedIn](https://www.linkedin.com/in/maïli-truong-6a9ab02b5)

## Fun facts 
---
- I have always been really competitive so I paricipated in piano, swimming, mathematics and taekwondo competitions.
- I love cinema and one of my favorite movies is 'Dead poets society'.
- I also love reading and my favorite book is 'To kill a mockingbird'.
- My favorite anime is 'Code Geass'.
- I studied Japanese on my own and now I know the basics of the language.
- I learned how to crochet, sew and wooven on my own so I can basically do any cloth I want.
- You can view my CV [here](/portfolio/cv.pdf) !


